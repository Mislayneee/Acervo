...
export default function Perfil() {
  ...
  const [abaAtiva, setAbaAtiva] = useState<"contribuicao" | "biblioteca">(
    "contribuicao"
  );

  // menu config (engrenagem)
  ...
  return (
    <main style={{ maxWidth: 1000, margin: "32px auto", padding: "0 24px" }}>
      {/* Barra superior com título e engrenagem */}
      <div
        style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}
      >
        <h2 style={{ margin: 0 }} className="h1">
          Perfil
        </h2>
      ...
      >
        <button
          ...
          style={tabBtn(abaAtiva === "contribuicao")}
        >
          Contribuição
        </button>
        <button
          onClick={() => setAbaAtiva("biblioteca")}
          style={tabBtn(abaAtiva === "biblioteca")}
        >
          Biblioteca Pessoal
        </button>
      </div>
      ...
      {abaAtiva === "contribuicao" && (
        <section className="perfil-page container" style={{ paddingBottom: 24 }}>
          {/* Título idêntico ao da Biblioteca Pessoal */}
          ...
          {/* Form mais largo, alinhado à mesma coluna da lista */}
          <form onSubmit={handleSubmit} style={{ maxWidth: "unset", marginInline: 0 }}>
            ...
            >
              <p>
                <strong>Arraste e solte ou clique para adicionar uma imagem</strong>
              </p>
              ...
              {formulario.imagem && (
                <div style={{ marginTop: 8, fontSize: 12 }}>
                  {formulario.imagem.name}
                </div>
              )}
            </div>

            {/* Botão menor, à direita (pill) */}
...
const Label: React.FC<{ children: React.ReactNode }> = ({ children }) => (
  <label style={{ display: "block", marginTop: 6, marginBottom: 6, fontWeight: 600 }}>
    {children}
  </label>
);
...
> = (props) => <textarea {...props} rows={4} style={inputStyle} />;
